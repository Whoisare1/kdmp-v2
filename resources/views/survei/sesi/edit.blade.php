    ...
    <div class="mx-auto max-w-2xl rounded-sm border border-paper-300 bg-paper-50 p-8">
        <form action="{{ route($routeBase . '.update', $item->id) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                @if($wilayahs->count() === 1)
                    <input type="hidden" name="id_petugas" value="{{ $item->id_petugas ?? auth()->id() }}">
                    <input type="text" value="{{ $item->petugas->nama ?? auth()->user()->nama ?? 'Petugas Survei' }}" readonly class="w-full rounded-sm border border-paper-300 bg-paper-100 px-3 py-2 text-sm text-ink-700 cursor-not-allowed font-medium">
                @else
                    <select name="id_petugas" id="id_petugas" required class="w-full rounded-sm border border-paper-300 bg-white px-3 py-2 text-sm focus:border-merah-400 focus:outline-none">
                        @foreach($petugasList as $petugas)
                            <option value="{{ $petugas->id }}" {{ old('id_petugas', $item->id_petugas) == $petugas->id ? 'selected' : '' }}>
                                {{ $petugas->nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-2 gap-4 mb-8">
                <div>
                    ...
                    <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $item->tahun) }}" required min="2000" max="2100" class="w-full rounded-sm border border-paper-300 bg-paper-100 px-3 py-2 text-sm focus:border-merah-400 focus:outline-none cursor-not-allowed" readonly>
                    @error('tahun')
                        <p class="mt-1 text-xs text-merah-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t border-paper-200 pt-5">
                ...
                <button type="submit" class="rounded-sm bg-merah-500 px-5 py-2 text-sm font-medium text-paper-50 hover:bg-merah-600 shadow-sm">Simpan Perubahan</button>
            </div>
        </form>
    </div>
