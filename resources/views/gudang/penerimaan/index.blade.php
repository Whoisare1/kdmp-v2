    ...
    <div class="mb-5 flex items-center justify-between gap-4">
        <form method="GET" class="flex items-center gap-2">
            <input
                ...
                name="q"
                value="{{ request('q') }}"
                placeholder="Cari kode / gudang / pihak..."
            ...
            <tbody>
                @forelse ($items as $item)
                    <tr class="border-b border-paper-200 last:border-0 hover:bg-paper-100/70">
                        <td class="px-4 py-3 font-medium text-ink-800">{{ $item->kode_penerimaan }}</td>
                        <td class="px-4 py-3 text-ink-800">{{ $item->tanggal_terima?->format('d M Y') }}</td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('gudang.penerimaan.show', $item->id_penerimaan) }}" class="text-xs font-medium text-ink-700 hover:text-merah-600">
                                Lihat
                            </a>
                        </td>
                    </tr>
